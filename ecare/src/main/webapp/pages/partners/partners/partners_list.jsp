...
<div class="container-fluid" id="main-container">
    ...
    <div id="main-content" class="clearfix">
        <div id="breadcrumbs">
            <ul class="breadcrumb">
                <li><i class="icon-home"></i> <a href="#">合作商管理</a><span class="divider"><i class="icon-angle-right"></i></span></li>
                <li class="active">合作商信息</li>
            </ul><!--.breadcrumb-->
            <div id="nav-search">
            </div><!--#nav-search-->
        </div><!--#breadcrumbs-->
        ...
        <div id="page-content" class="clearfix">
            <div class="row-fluid">
                <div class="row-fluid">
                    <!-- 检索  -->
                    <form action="${pageContext.request.contextPath}/partners/partners/findList.do?am=${mPd.am }&bm=${mPd.bm }" method="post" name="userForm" id="userForm">
                        <div class="page-header position-relative">
                            <table>
                                <tr>
                                    <td><font color="#808080">登录账号：</font></td>
                                    ...
                                    <%--<td>--%>
                                        <%--<select style="width:100%;" name="type" id="type">--%>
                                            <%--<option value="">请选择合作商类型</option>--%>
                                            <%--<c:forEach items="${dictionaryList}" var="v">--%>
                                                <%--<option value="${v.id}" <c:if test="${pd.type eq v.id}">selected="selected"</c:if>>${v.name}</option>--%>
                                            <%--</c:forEach>--%>
                                        <%--</select>--%>
                                    <%--</td>--%>
                            ...
                            <c:if test="${fn:contains(power,'partners_info_add')}">
                                <a class="btn btn-success btn-sm" onclick="add()">添加</a>
                            </c:if>
                            <c:if test="${fn:contains(power,'partners_info_edit')}">
                                <a class="btn btn-warning btn-sm" onclick="edits()">编辑</a>
                            </c:if>
                            <c:if test="${fn:contains(power,'partners_info_del')}">
                            ...
                            <c:if test="${fn:contains(power,'partners_info_trash')}">
                                <a class="btn btn-sm" onclick="findTrash()">回收站(${pd.num })</a>
                            </c:if>
                            <c:if test="${fn:contains(power,'partners_info_server')}">
                                <a class="btn btn-info btn-sm" onclick="setServerTypes()">设置服务分类</a>
                            </c:if>
                                            ...
                                            <c:when test="${not empty userList}">
                                                <c:forEach items="${userList}" var="var" varStatus="vs">
                                                    <tr ondblclick="findInfo('${var.id }')">
                                                        ...
                                                        <td>${var.phone}</td>
                                                        <td>
                                                        	<c:if test="${var.is_authent=='N'}">未认证</c:if>
                                                        	<c:if test="${var.is_authent=='Y'}">认证通过</c:if>
                                                        	<%--<c:if test="${var.is_authent=='2'}">认证不通过</c:if>--%>
                                                        </td>
                                                        <c:if test="${fn:contains(power,'user_stauts')}">
...
<script type="text/javascript">
    ...
    //新增
    function add(){
        art.dialog.open('${pageContext.request.contextPath}/partners/partners/toadd.do',{
            title:'新增',
            width:820,
            height:500,
            lock: true
    ...
    //修改
    function edit(id){
        art.dialog.open('${pageContext.request.contextPath}/partners/partners/toadd.do?id='+id,{
            title:'编辑',
            width:820,
            height:500,
            lock: true
    ...
    function del(id){
        bootbox.confirm("确定要删除该数据?", function(result) {
            if(result) {
                var url = "${pageContext.request.contextPath}/partners/partners/del.do?ids="+id;
    ...
    function edits(){
        ...
        if(data.length==1){
            art.dialog.open('${pageContext.request.contextPath}/partners/partners/toadd.do?id='+data.val(),{
                title:'编辑',
                width:820,
                height:500,
                lock: true
    ...
    function dels(){
        ...
        if(data.length>0){
            ...
            bootbox.confirm("确定要删除选中的数据?", function(result) {
                if(result) {
                    var url = "${pageContext.request.contextPath}/partners/partners/del.do?ids="+ids;
                    $.get(url,function(data){
                        if(data){
                            saveSuccess();
                        }
    ...
    function findInfo(id){
        ...
        var data = $('input:checkbox[id^=check_]:checked');
        if(data.length==1){
            art.dialog.open('${pageContext.request.contextPath}/partners/partners/findInfo.do?id='+data.val(),{
                title:'个人信息',
                width:550,
                height:500,
                lock: true,
    ...
    //回收站
    function findTrash(){
        art.dialog.open('${pageContext.request.contextPath}/partners/partners/findTrash.do',{
            title:'回收站',
            width:1000,
            height:520,
            lock: true,
    ...
    function updateStatus(id){
        ...
        $.ajax({
            type: "GET",
            url: "${pageContext.request.contextPath}/partners/partners/updateStatus.do?id="+id+"&status="+flag,
            data: "",
            dataType: "json",
    ...
    function setServerTypes(){
        var data = $('input:checkbox[id^=check_]:checked');
        if(data.length==1){
            art.dialog.open('${pageContext.request.contextPath}/partners/partners/toUpdateServerCategory.do?user_id='+data.val(),{
                title:'设置服务类型',
                width:350,
                height:500,
                lock: true
...
//    }
	...
	function setRole(){
		var data = $('input:checkbox[id^=check_]:checked');
		if(data.length==1){
			art.dialog.open('${pageContext.request.contextPath}/common/commonRole/findUserRole.do?user_id='+data.val()+'&classify=hzs',{
		   	 	title:'设置用户角色',
				width:550,
		    	height:500,
	    		lock: true
