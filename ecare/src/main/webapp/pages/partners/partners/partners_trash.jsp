...
<div id="page-content" class="clearfix">
    <div class="row-fluid">
        <div class="row-fluid">
            <!-- 检索  -->
            <form action="${pageContext.request.contextPath}/callCenter/telemarketer/findTrash.do?am=${mPd.am }&bm=${mPd.bm }" method="post" name="userForm" id="userForm">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <c:if test="${var.status=='Y'}">
                                            <label><input id="status_${var.id }" name="switch-field-1" class="ace-switch ace-switch-6" type="checkbox" checked="checked" disabled="disabled"/><span class="lbl"></span></label>
                                        </c:if>
                                        <c:if test="${var.status=='N'}">
                                            <label><input id="status_${var.id }" name="switch-field-1" class="ace-switch ace-switch-6" type="checkbox" disabled="disabled"/><span class="lbl"></span></label>
                                        </c:if>
                                    </td>
...
<script type="text/javascript">
    ...
    function myClear(){
        ...
        if(data.length>0){
            ...
            bootbox.confirm("确定要清除选中的数据吗?清除之后不可恢复！", function(result) {
                if(result) {
                    var url = "${pageContext.request.contextPath}/callCenter/telemarketer/clearUser.do?ids="+ids+"&flag=clear";
    ...
    function revert(){
        ...
        if(data.length>0){
            ...
            bootbox.confirm("确定要还原选中的数据吗？", function(result) {
                if(result) {
                    var url = "${pageContext.request.contextPath}/callCenter/telemarketer/revertUser.do?ids="+ids+"&flag=revert";
                    $.get(url,function(data){
                        if(data){
                            //document.location.reload();
                            nextPage('${page.currentPage}');
    ...
    function all_clear(){
        bootbox.confirm("确定要清除全部数据吗?清除之后不可恢复！", function(result) {
            if(result) {
                var url = "${pageContext.request.contextPath}/partners/partners/clearUser.do?flag=all_clear";
                $.get(url,function(data){
                    if(data){
                        //document.location.reload();
                        nextPage('${page.currentPage}');
    ...
    function all_revert(){
        bootbox.confirm("确定要还原全部数据吗？", function(result) {
            if(result) {
                var url = "${pageContext.request.contextPath}/partners/partners/revertUser.do?flag=all_revert";
                $.get(url,function(data){
                    if(data){
                        //document.location.reload();
                        nextPage('${page.currentPage}');
    ...
    function findInfo(id){
        art.dialog.open('${pageContext.request.contextPath}/partners/partners/findInfo.do?id='+id,{
            title:'查看详情',
            width:550,
            height:500,
            lock: true
