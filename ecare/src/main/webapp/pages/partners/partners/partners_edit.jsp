            ...
            $("#cdForm").validate({
                rules: {
                    login_name: {
                        ...
                        rangelength:[4,18],
                        remote: {
                            url: "${pageContext.request.contextPath}/system/user/checkLoginName.do?flag=${p.login_name }",     //后台处理程序
                            type: "post",               //数据发送方式
...
<body>
<form action="${pageContext.request.contextPath}/partners/partners/update.do" name="cdForm" id="cdForm" method="post" enctype="multipart/form-data">
    <c:if test="${pd.label!='b'}">
        <div id="zhongxin">
            <table width="90%">
                <tr>
                    <td width="20%" align="right">登录账号：</td>
                    <td><input type="text" name="login_name" id="login_name" value="${p.login_name }" placeholder="这里输入登录名称" title="登录名称"/></td>
                </tr>
                <tr>
                    <td width="20%" align="right">登录密码：</td>
                    <td><input type="password" name="login_password" id="login_password" value="" placeholder="这里输入登录密码" title="登录密码"/></td>
                </tr>
                <tr>
                    <td width="20%" align="right">客服名称：</td>
                    <td><input type="text" name="user_name" id="user_name" value="${p.user_name }" placeholder="这里输入用户名称" title="用户名称"/></td>
                </tr>
                    <tr>
                        <td width="20%" align="right">电子邮箱：</td>
                        <td><input type="text" name="email" id="email" value="${p.email }" placeholder="这里输入电子邮箱" title="电子邮箱"/></td>
                    </tr>
                    <tr>
                        <td width="20%" align="right">联系方式：</td>
                        <td><input type="text" name="phone" id="phone" value="${p.phone }" placeholder="这里输入联系方式" title="联系方式"/></td>
                    </tr>
                <tr>
                    <td width="20%" align="right">合作商类型：</td>
                    <td>
                        <select style="width:100%;" name="partner_type" id="partner_type">
                            <c:forEach items="${dictionaryList}" var="v">
                                <option value="${v.id}" <c:if test="${p.partner_type eq v.id}">selected="selected"</c:if>>${v.name}</option>
                            </c:forEach>
                        </select>
                    </td>
                </tr>
                <tr>
                    ...
                    <td><img alt="" src="${p.head_url }" style="width: 80px;height: 80px;margin-bottom:10px;"><br/>
                        <input type="file" name="head_address" id="head_address" value="${p.head_address }"/></td>
                </tr>
                <tr>
                    <td width="20%" align="right">合作商地址：</td>
                    <td>
                        <input type="text" name="live_address" id="live_address" value="${p.live_address }"  placeholder="这里输入合作商地址" title="合作商地址"/>
                    </td>
                </tr>
                <tr>
                    <td width="20%" align="right">排序：</td>
                    <td><input type="text" name="order_by" id="order_by" value="${p.order_by }" placeholder="这里输入排序" title="排序"/></td>
                </tr>
                <%--<tr>--%>
                ...
                <tr>
                    <td style="text-align: center;" colspan="2">
                        <input type="hidden" name="head_address_old" value="${p.head_address }">
