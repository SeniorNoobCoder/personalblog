...
//            zTree.setting.check.chkboxType = { "Y" : "ps", "N" : "ps" };
        ...
        $(document).ready(function(){
            var treeNodes = '';
            $.ajax({
                url : "${pageContext.request.contextPath}/partners/business/findServerCategory.do?id="+<%=business_id%>,
                data : {},
                type : 'GET',
                dataType : 'json',
    ...
    <script type="text/javascript">
        ...
        var url="${pageContext.request.contextPath}/partners/business/saveServerCategory.do?user_id="+user_id;
        function save(lx){
        	if(lx=='save_before'){
        		window.location.href="${pageContext.request.contextPath}/partners/partners/toaddAuthent.do?partner_id="+user_id;
        	}else{
	            ...
	            if(nodes.length<1){
	                ...
	                $.ajax({
	                    url : url+"&category="+v+"&categoryName="+name+"&&business_id=<%=business_id%>",
	                    data : {},
	                    type : 'GET',
	                    success : function(data) {
	                        art.dialog.alert("服务分类设置成功。");
	                        //var win = art.dialog.open.origin;
	                        //win.location.reload();
//	                        art.dialog.open.api.close();
//	                        art.dialog.opener.resh();
	                    },
	                    error : function(xhr, status) {
	                        alert('重新设置服务商服务分类失败!');
	                    },
...
<%--<div align="center">--%>
	<%--&lt;%&ndash;<a class="btn btn-mini btn-primary" onclick="save('save_before');">上一步</a>&ndash;%&gt;--%>
