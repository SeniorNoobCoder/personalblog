...
<html lang="en">
  ...
  <body>
    <form action="${pageContext.request.contextPath}/system/area/update.do" name="cdForm" id="cdForm" method="post" enctype="multipart/form-data">
		...
		<table width="90%">
			<tr>
				...
				<c:if test="${p.parent_name==''||p.parent_name==null}">
					<td><input type="text" style="width: 95%" value="地区管理" readonly="readonly" title="上级地区名称"/></td>
				</c:if>
				<c:if test="${p.parent_name!=''&&p.parent_name!=null}">
					<td><input type="text" style="width: 95%" value="${p.parent_name }" readonly="readonly" title="上级地区名称"/></td>
				</c:if>
			</tr>
			<tr>
				<td width="20%" align="right">地区名称：</td>
				<td><input type="text" name="name" id="name" style="width: 95%" value="${p.name }" placeholder="这里输入地区名称" title="地区名称"/></td>
			</tr>
			<tr>
