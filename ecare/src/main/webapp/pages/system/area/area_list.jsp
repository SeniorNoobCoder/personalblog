			...
			<div id="main-content" class="clearfix">
					...
					<div id="page-content" class="clearfix">
						  <div class="row-fluid">
							<div class="row-fluid">
									<!-- 检索  -->
									<form action="${pageContext.request.contextPath}/system/area/findList.do?am=${mPd.am }&bm=${mPd.bm }" method="post" name="areaForm" id="areaForm">
									<div class="page-header position-relative">
										<table>
											<tr>
<!--												<td><font color="#808080">地区名称：</font></td>-->
<!--												<td><input type="text" name="name" value="${pd.name }" placeholder="这里输入地区名称" style="width:130px;"/></td>-->
<!--												<td style="vertical-align:top;"><button class="btn btn-mini btn-light" onclick="search();"><i id="nav-search-icon" class="icon-search"></i></button></td>-->
		...
		<script type="text/javascript">
			...
			//新增
			function add(){
				if($("#tree_id").val()==''||$("#tree_id").val()==null){
					art.dialog.alert("请在左侧地区树中选择父节点。");
				}else{
					art.dialog.open('${pageContext.request.contextPath}/system/area/toadd.do?parent_id='+$("#tree_id").val()+"&parent_name="+$("#tree_name").val()+"&parent_type="+$("#tree_type").val(),{
				   	 	title:'新增',
						width:550,
			    		height:500,
			    		lock: true
			...
			//修改
			function edit(id){
				art.dialog.open('${pageContext.request.contextPath}/system/area/toedit.do?id='+id,{
			   	 	title:'编辑',
					width:550,
		    		height:500,
		    		lock: true
			...
			function del(id){
				bootbox.confirm("确定要删除该数据?", function(result) {
					if(result) {
						var url = "${pageContext.request.contextPath}/system/area/del.do?ids="+id;
			...
			function edits(){
				...
				if(data.length==1){
					art.dialog.open('${pageContext.request.contextPath}/system/area/toedit.do?id='+data.val(),{
				   	 	title:'编辑',
						width:550,
			    		height:500,
			    		lock: true
			...
			function dels(){
				...
				if(data.length>0){
					...
					if(result) {
						var url = "${pageContext.request.contextPath}/system/area/del.do?ids="+ids;
						$.get(url,function(data){
							if(data){
								//document.location.reload();
								nextPage('${page.currentPage}');
			...
			function findInfo(id){
				var data = $('input:checkbox[id^=check_]:checked');
				if(data.length==1){
					art.dialog.open('${pageContext.request.contextPath}/system/area/findInfo.do?id='+data.val(),{
				   	 	title:'查看详情',
						width:550,
			    		height:500,
			    		lock: true
			...
			//回收站
			function findTrash(){
				art.dialog.open('${pageContext.request.contextPath}/system/area/findTrash.do',{
			   	 	title:'回收站',
					width:1000,
		    		height:520,
		    		lock: true,
			...
			function updateStatus(id){
				...
				$.ajax({
		            type: "GET",
		            url: "${pageContext.request.contextPath}/system/area/updateStatus.do?id="+id+"&status="+flag,
		            data: "",
		            dataType: "json",
		...
		$exportLink.addEventListener('click', function(e){
			...
			if(e.target.nodeName === "A"){
				tableExport('table_report', '地区信息', e.target.getAttribute('data-type'));
			}
