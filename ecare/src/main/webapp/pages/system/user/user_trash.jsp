  ...
  <body>
    <div id="page-content" class="clearfix">
	  <div class="row-fluid">
		<div class="row-fluid">
				<!-- 检索  -->
				<form action="${pageContext.request.contextPath}/system/user/findTrash.do?am=${mPd.am }&bm=${mPd.bm }" method="post" name="userForm" id="userForm">
	...
	<script type="text/javascript">
		...
		function myClear(){
			...
			if(data.length>0){
				...
				if(result) {
					var url = "${pageContext.request.contextPath}/system/user/clearUser.do?ids="+ids+"&flag=clear";
		...
		function revert(){
			...
			if(data.length>0){
				...
				if(result) {
					var url = "${pageContext.request.contextPath}/system/user/revertUser.do?ids="+ids+"&flag=revert";
					$.get(url,function(data){
						if(data){
							//document.location.reload();
							nextPage('${page.currentPage}');
		...
		function all_clear(){
			bootbox.confirm("确定要清除全部数据吗?清除之后不可恢复！", function(result) {
				if(result) {
					var url = "${pageContext.request.contextPath}/system/user/clearUser.do?flag=all_clear";
					$.get(url,function(data){
						if(data){
							//document.location.reload();
							nextPage('${page.currentPage}');
		...
		function all_revert(){
			bootbox.confirm("确定要还原全部数据吗？", function(result) {
				if(result) {
					var url = "${pageContext.request.contextPath}/system/user/revertUser.do?flag=all_revert";
					$.get(url,function(data){
						if(data){
							//document.location.reload();
							nextPage('${page.currentPage}');
