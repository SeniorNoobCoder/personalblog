			...
			<div id="main-content" class="clearfix">
					...
					<div id="page-content" class="clearfix">
						  <div class="row-fluid">
							<div class="row-fluid">
									<!-- 检索  -->
									<form action="${pageContext.request.contextPath}/system/user/findList.do?am=${mPd.am }&bm=${mPd.bm }" method="post" name="userForm" id="userForm">
									...
									<c:if test="${fn:contains(power,'user_configOrg')}">
										<a class="btn btn-info btn-sm" onclick="setOrganize()">设置组织</a>
									</c:if>
									</div>
									<!-- 检索  -->
...
<!--										<td style="vertical-align:top;"><a class="btn btn-small btn-success" onclick="add();">新增</a></td>-->
		...
		<script type="text/javascript">
			...
			//新增
			function add(){
				 art.dialog.open('${pageContext.request.contextPath}/system/user/toadd.do',{
			   	 	title:'新增',
					width:550,
		    		height:500,
		    		lock: true
			...
			function del(id){
				bootbox.confirm("确定要删除该数据?", function(result) {
					if(result) {
						var url = "${pageContext.request.contextPath}/system/user/del.do?ids="+id;
			...
			function edits(){
				...
				if(data.length==1){
					art.dialog.open('${pageContext.request.contextPath}/system/user/toedit.do?id='+data.val(),{
				   	 	title:'编辑',
						width:550,
			    		height:500,
			    		lock: true
			...
			function dels(){
				...
				if(data.length>0){
					...
					if(result) {
						var url = "${pageContext.request.contextPath}/system/user/del.do?ids="+ids;
						$.get(url,function(data){
							if(data){
								saveSuccess();
							}
			...
			function findInfo(id){
				var data = $('input:checkbox[id^=check_]:checked');
				if(data.length==1){
					art.dialog.open('${pageContext.request.contextPath}/system/user/findInfo.do?id='+data.val(),{
				   	 	title:'查看详情',
						width:550,
			    		height:500,
			    		lock: true,
			...
			function findInfoById(id){
				art.dialog.open('${pageContext.request.contextPath}/system/user/findInfo.do?id='+id,{
					title:'用户信息',
					width:550,
					height:200,
					lock: true,
			...
			//回收站
			function findTrash(){
				art.dialog.open('${pageContext.request.contextPath}/system/user/findTrash.do',{
			   	 	title:'回收站',
					width:1000,
		    		height:520,
		    		lock: true,
			...
			function setRole(){
				var data = $('input:checkbox[id^=check_]:checked');
				if(data.length==1){
					art.dialog.open('${pageContext.request.contextPath}/system/user/findUserRole.do?user_id='+data.val(),{
				   	 	title:'设置用户角色',
						width:550,
				    	height:500,
			    		lock: true
			...
			function setOrganize(){
				var data = $('input:checkbox[id^=check_]:checked');
				if(data.length==1){
					art.dialog.open('${pageContext.request.contextPath}/system/user/findUserOrganize.do?user_id='+data.val(),{
				   	 	title:'设置用户组织',
						width:350,
				    	height:500,
			    		lock: true
