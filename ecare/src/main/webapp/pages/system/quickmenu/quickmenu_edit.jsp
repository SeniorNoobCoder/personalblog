				...
				$.ajax({
		            ...
		            success: function(data){
		            	...
		            	$.each(data,function(i,n){
		            		if(two_menu==n.id){
		            			tt += '<option value="'+n.id+'" title="'+n.code+'" selected="selected" >'+n.name+'</option>';
		            		}else{
...
<body>
	<c:if test="${pd.flag!='b'}">
		<form action="${pageContext.request.contextPath}/system/quickmenu/update.do" name="cdForm" id="cdForm" method="post" enctype="multipart/form-data">
			<div id="zhongxin">
			<table width="90%">
				<tr>
					<td width="20%" align="right">一级菜单：</td>
					<td>
						<select name="one_menu" id="one_menu" style="width: 98%" onchange="selectOneMenu();">
							<option value="">请选择</option>
							<c:forEach items="${list}" var="var" varStatus="vs">
								<option value="${var.id }" <c:if test="${var.id==p.one_menu }"> selected="selected" </c:if>>${var.name }</option>
							</c:forEach>
						</select>
					</td>
				</tr>
				...
				<td width="20%" align="right">二级菜单：</td>
						<td>
							<select name="two_menu" id="two_menu" style="width: 98%" onchange="selectUnit();">
							</select>
						</td>
				</tr>
				<tr>
					...
					<td>
					<select id="level" style="width: 98%" disabled="disabled">
						<option value="1" <c:if test="${p.level==1 }"> selected="selected" </c:if> >1</option>
						<option value="2" <c:if test="${p.level==2 }"> selected="selected" </c:if> >2</option>
						<option value="3" <c:if test="${p.level==3 }"> selected="selected" </c:if> >3</option>
						<option value="4" <c:if test="${p.level==4 }"> selected="selected" </c:if> >4</option>
					</select>
					</td>
				</tr>
				<tr>
