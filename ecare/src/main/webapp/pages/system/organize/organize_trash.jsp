...
<html lang="en">
  ...
  <body>
    ...
    <div class="row-fluid">
			<!-- 检索  -->
			<form action="${pageContext.request.contextPath}/system/organize/findTrash.do?am=${mPd.am }&bm=${mPd.bm }" method="post" name="organizeForm" id="organizeForm">
			<!-- 检索  -->
				...
				<c:choose>
					<c:when test="${not empty organizeList}">
						<c:forEach items="${organizeList}" var="organize" varStatus="vs">
							<tr>
								<td style="width: 20px;"><input type="checkbox" style="opacity: 1;" id="check_${organize.id }" value="${organize.id }" onchange="myCheck('${organize.id }');"></td>
								<td>公司分管领导</td>
								<td>${organize.name }</td>
	...
	function myClear(){
			...
			if(data.length>0){
				...
				if(result) {
					var url = "${pageContext.request.contextPath}/system/organize/clearOrganize.do?ids="+ids+"&flag=clear";
		...
		function revert(){
			...
			if(data.length>0){
				...
				if(result) {
					var url = "${pageContext.request.contextPath}/system/organize/revertOrganize.do?ids="+ids+"&flag=revert";
					$.get(url,function(data){
						if(data){
							//document.location.reload();
							nextPage('${page.currentPage}');
		...
		function all_clear(){
			bootbox.confirm("确定要清除全部数据吗?清除之后不可恢复！", function(result) {
				if(result) {
					var url = "${pageContext.request.contextPath}/system/organize/clearOrganize.do?flag=all_clear";
					$.get(url,function(data){
						if(data){
							//document.location.reload();
							nextPage('${page.currentPage}');
		...
		function all_revert(){
			bootbox.confirm("确定要还原全部数据吗？", function(result) {
				if(result) {
					var url = "${pageContext.request.contextPath}/system/organize/revertOrganize.do?flag=all_revert";
					$.get(url,function(data){
						if(data){
							//document.location.reload();
							nextPage('${page.currentPage}');
		...
		var name = $("#tree_name").val();
			art.dialog.open('${pageContext.request.contextPath}/system/organize/findInfo.do?id='+id+'&name='+name,{
		   	 	title:'详情',
				width:550,
	    		height:500,
	    		lock: true
