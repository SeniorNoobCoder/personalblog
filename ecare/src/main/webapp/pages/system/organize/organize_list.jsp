<%@ page language="java" import="java.util.*" pageEncoding="UTF-8"%>
	<%@include file="../admin/top.jsp" %>
		...
		<div class="container-fluid" id="main-container">
			...
			<div id="main-content" class="clearfix">
					...
					<div id="page-content" class="clearfix">
						<div class="row-fluid">
							<div class="row-fluid">
							<form action="${pageContext.request.contextPath}/system/organize/findList.do?am=${mPd.am }&bm=${mPd.bm }" method="post" name="organizeForm" id="organizeForm">
									<div class="page-header position-relative">
									<c:if test="${fn:contains(power,'organize_add')}">
										<a class="btn btn-success btn-sm" onclick="addMenu()">添加</a>
									</c:if>
									<c:if test="${fn:contains(power,'organize_edit')}">
										<a class="btn btn-warning btn-sm" onclick="edits()">编辑</a>
									</c:if>
									<c:if test="${fn:contains(power,'organize_del')}"> 
										<a class="btn btn-danger btn-sm" onclick="dels()">删除</a>
									</c:if>
									<c:if test="${fn:contains(power,'organize_info')}">
									...
									<table id="table_report" class="table table-striped table-bordered table-hover">
										...
										<!-- 开始循环 -->	
										<c:choose>
											<c:when test="${not empty organizeList}">
												<c:forEach items="${organizeList}" var="organize" varStatus="vs">
													<tr>
														<td id="exclude_0_${organize.id }" style="width:20px;"><input type="checkbox" style="opacity: 1;" id="check_${organize.id }" value="${organize.id }" onchange="myCheck('${organize.id }');"></td>
														<td>公司分管领导</td>
														<td>${organize.name }</td>
														<td>${organize.short_name }</td>
														<td>${organize.order_by}</td>
														<td><fmt:formatDate value="${organize.create_time }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
														<td style="" id="exclude_1_${var.id }">
														<c:if test="${fn:contains(power,'organize_edit')}">
															<a class='btn btn-mini btn-info' title="编辑" onclick="edit('${organize.id }');"><i class='icon-edit'></i></a>
														</c:if>
														<c:if test="${fn:contains(power,'organize_del')}">
															<a class='btn btn-mini btn-danger' title="删除" onclick="del('${organize.id }');"><i class='icon-trash'></i></a>
														</c:if>
														</td>
													</tr>
												</c:forEach>
		...
		function myCheckAll(){
			...
			function addMenu(){
				...
				alert("请在左侧选择菜单父节点。");
					...
					art.dialog.open('${pageContext.request.contextPath}/system/organize/toadd.do?parent_id='+parent_id+'&name='+name+'&level='+level+'&org_cascade='+org_cascade,{
				   	 	title:'新增',
						width:550,
			    		height:400,
			    		lock: true
			...
			function edit(id){
			var name = $("#tree_name").val();
				art.dialog.open('${pageContext.request.contextPath}/system/organize/toedit.do?id='+id+'&name='+name,{
			   	 	title:'编辑',
					width:550,
		    		height:500,
		    		lock: true
			...
			var name = $("#tree_name").val();
				...
				if(data.length==1){
					art.dialog.open('${pageContext.request.contextPath}/system/organize/toedit.do?id='+data.val()+'&name='+name,{
				   	 	title:'编辑',
						width:550,
			    		height:500,
			    		lock: true
			...
			var name = $("#tree_name").val();
				var data = $('input:checkbox[id^=check_]:checked');
				if(data.length==1){
					art.dialog.open('${pageContext.request.contextPath}/system/organize/findInfo.do?id='+data.val()+'&name='+name,{
				   	 	title:'查看详情',
						width:550,
			    		height:350,
			    		lock: true,
			...
			function del(id){
					bootbox.confirm("确定要删除该记录?", function(result) {
						if(result) {
							var url = "${pageContext.request.contextPath}/system/organize/del.do?ids="+id;
			...
			function dels(){
				...
				if(data.length>0){
					...
					if(result) {
						var url = "${pageContext.request.contextPath}/system/organize/del.do?ids="+ids;
						$.get(url,function(data){
							if(data){
								saveSuccess();
							}
		...
		function findDustbin(){
			art.dialog.open('${pageContext.request.contextPath}/system/organize/findTrash.do',{
		   	 	title:'回收站',
				width:1000,
		    	height:520,
	    		lock: true,
		...
		$exportLink.addEventListener('click', function(e){
			...
			if(e.target.nodeName === "A"){
				tableExport('table_report', '组织信息', e.target.getAttribute('data-type'));
			}
