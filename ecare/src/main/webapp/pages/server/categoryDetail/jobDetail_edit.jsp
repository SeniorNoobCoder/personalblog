...
<body>
<form action="${pageContext.request.contextPath}/server/categoryDetail/updateJobDetail.do" name="cdForm" id="cdForm" method="post" enctype="multipart/form-data">
        ...
        <div id="zhongxin">
            <table width="95%">
                ...
                <tr>
                    ...
                    <td>
                        <textarea rows="2" cols="8" name="remark" id="remark" placeholder="这里输入工作内容" title="工作内容">${pd.remark}</textarea>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <select  name="pay_type" id="pay_type" onchange="isShow()">
                            <option value="0" <c:if test="${pd.pay_type=='0'}">selected="selected"</c:if>>固定收费</option>
                            <option value="1" <c:if test="${pd.pay_type=='1'}">selected="selected"</c:if>>按时收费</option>
                        </select>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <input type="text" name="starting_price" id="starting_price" placeholder="这里输入起始价格" title="起始价格" style="width: 80%" value="${pd.starting_price }"
                               onkeypress="if(!this.value.match(/^[\+\-]?\d*?\.?\d*?$/))this.value=this.t_value;else  this.t_value=this.value;if(this.value.match(/^(?:[\+\-]?\d+(?:\.\d+)?)?$/))this.o_value=this.value"  onkeyup="if(!this.value.match(/^[\+\-]?\d*?\.?\d*?$/))this.value=this.t_value;else  this.t_value=this.value;if(this.value.match(/^(?:[\+\-]?\d+(?:\.\d+)?)?$/))this.o_value=this.value"  onblur="if(!this.value.match(/^(?:[\+\-]?\d+(?:\.\d+)?|\.\d*?)?$/))this.value=this.o_value;else{if(this.value.match(/^\.\d+$/))this.value=0+this.value;if(this.value.match(/^\.$/))this.value=0;this.o_value=this.value}">
                        元起
                    </td>
                </tr>
                ...
                <tr id = "charge_mode">
                    ...
                    <td> 每 <input type="text" name="charge_mode_time" id="charge_mode_time" placeholder="计费标准" title="计费标准" style="width: 50px" value="${pd.charge_mode_time }"
                                  onkeypress="if(!this.value.match(/^[\+\-]?\d*?\.?\d*?$/))this.value=this.t_value;else  this.t_value=this.value;if(this.value.match(/^(?:[\+\-]?\d+(?:\.\d+)?)?$/))this.o_value=this.value"  onkeyup="if(!this.value.match(/^[\+\-]?\d*?\.?\d*?$/))this.value=this.t_value;else  this.t_value=this.value;if(this.value.match(/^(?:[\+\-]?\d+(?:\.\d+)?)?$/))this.o_value=this.value"  onblur="if(!this.value.match(/^(?:[\+\-]?\d+(?:\.\d+)?|\.\d*?)?$/))this.value=this.o_value;else{if(this.value.match(/^\.\d+$/))this.value=0+this.value;if(this.value.match(/^\.$/))this.value=0;this.o_value=this.value}">
                        <input type="text" name="charge_mode_unit" id="charge_mode_unit" placeholder="单位" title="单位" style="width: 50px" value="${pd.charge_mode_unit }"/>
                        收费 <input type="text" name="price" id="price" placeholder="单价" title="单价" style="width: 50px" value="${pd.price }"
                                  onkeypress="if(!this.value.match(/^[\+\-]?\d*?\.?\d*?$/))this.value=this.t_value;else  this.t_value=this.value;if(this.value.match(/^(?:[\+\-]?\d+(?:\.\d+)?)?$/))this.o_value=this.value"  onkeyup="if(!this.value.match(/^[\+\-]?\d*?\.?\d*?$/))this.value=this.t_value;else  this.t_value=this.value;if(this.value.match(/^(?:[\+\-]?\d+(?:\.\d+)?)?$/))this.o_value=this.value"  onblur="if(!this.value.match(/^(?:[\+\-]?\d+(?:\.\d+)?|\.\d*?)?$/))this.value=this.o_value;else{if(this.value.match(/^\.\d+$/))this.value=0+this.value;if(this.value.match(/^\.$/))this.value=0;this.o_value=this.value}">
                        元
                    </td>
                </tr>
                ...
                <tr>
                    <td style="text-align: center;" colspan="2">
                        <input type="hidden" name="id" id="id" title="父类id" value="${pd.id }"/>
