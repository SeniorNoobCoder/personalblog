...
<head>
    ...
    <script type="text/javascript">
        ...
        function save(){
            ...
            bootbox.confirm("确定要提交提单?", function(result) {
                if(result) {
                    $("#cdForm").submit();
                }
            });
...
<body>
<form action="${pageContext.request.contextPath}/order/callRecord/saveOrderByCall.do" name="cdForm" id="cdForm" method="post" enctype="multipart/form-data">
    <c:if test="${pd.label!='b'}">
        <div id="zhongxin">
            <div style="width: 100%;text-align: center;margin: 10px 0 10px 0;font-size: 20px; color: #2a91d8">客户信息</div>
            <table id="infoTable" class="table table-striped table-bordered table-hover" >
                <tr>
                    ...
                    <td width="90px"><span style="color: #00b4f5">设备编号：</span></td>
                    <td>
                    ${customer.sn}
                ...
                <tr>
                    <td><span style="color: #00b4f5">用户名称：</span></td>
                    <td>${customer.name}
                        <input type="hidden" name="customer_name" id="customer_name" value="${customer.name}">
                    </td>
                </tr>
                <tr>
                    <td><span style="color: #00b4f5">性&nbsp;&nbsp;&nbsp;&nbsp;别：</span></td>
                    <td>${customer.sex}</td>
                </tr>
                <tr>
                    <td><span style="color: #00b4f5">联系方式：</span></td>
                    <td>${customer.phone}
                        <input type="hidden" name="customer_phone" id="customer_phone" value="${customer.phone}">
                    </td>
                </tr>
                <tr>
                    <td><span style="color: #00b4f5">家庭住址：</span></td>
                    <td colspan="2">${customer.address}
                        <input type="hidden" name="customer_address" id="customer_address" value="${customer.address}">
                    </td>
                </tr>
            </table>
            <div style="width: 100%;text-align: center;margin: 0 0 10px 0;font-size: 20px; color: #2a91d8">订单信息</div>
            <table width="90%">
                <tr>
                    <td width="20%" align="right">订单类型：</td>
                    <td>
...
<div class="modal hide fade" id="myModal" tabindex="-1" role="dialog">
    ...
    <%--&lt;%&ndash;<button class="close" type="button" data-dismiss="modal">×</button>&ndash;%&gt;--%>
    <%--<h3 id="myModalLabel" style="color: red">此记录已被受理，不能重复受理</h3>--%>
    <%--</div>--%>
    <div class="modal-body">
        ...
        <div  style="text-align: right">
            <a href="#" class="btn" onclick="art.dialog.opener.saveSuccess()">关闭</a>
        </div>
    </div>
</div>
<!-- 引入 -->
