...
<head>
    ...
    <script type="text/javascript">
        ...
        function save(){
            //url 默认是根据电话记录添加
            var url = "${pageContext.request.contextPath}/order/order/saveOrderByCall.do";
            var addType = "${pd.addType}";
            if(addType == "customer"){//根据用户记录添加订单
                url = "${pageContext.request.contextPath}/order/order/saveOrderByCustomer.do";
            }
            if (! $("#cdForm").valid()) {
                return;
            }
            ...
            <div id="order" class="order">
                ...
                <div id="info">
                    <div style="float: left;width: 30%">
                        <c:if test="${customer.head_address == ''}">
                            <img src="/IMAGES/index-title1.jpg" width="80%"/>
                        </c:if>
                        <c:if test="${customer.head_address != ''}">
                            <img src="${customer.head_address}" width="80%"/>
                        </c:if>
                    </div>
                    <div style="float: left;width: 65%">
                        <ul>
                            ...
                            <input type="hidden" id="customer_sn" name="customer_sn" value="${customer.sn}"/>
                            </li>
                            <li>名称：${customer.name}&nbsp;&nbsp;性别：${customer.sex}&nbsp;&nbsp;生日：${customer.birthday}
                                <input type="hidden" id="customer_name" name="customer_name" value="${customer.name}"/>
                            </li>
                            <li>电话：${customer.phone}
                                <input type="hidden" id="customer_phone" name="customer_phone" value="${customer.phone}"/>
                            </li>
                            <li>家庭住址:${customer.address}
                                <input type="hidden" id="customer_address" name="customer_address" value="${customer.address}"/>
                            </li>
                            <li>健康状况:${customer.health_condition}</li>
                        </ul>
                    </div>
                    <div style=" clear:both"></div>
                </div>
            </div>
            <div id="showTitle">订单信息</div>
            <table width="90%">
                <tr>
...
//            endDate.minDate = datas; //开始日选好后，重置结束日的最小日期
    ...
    $('#appointment_time').jeDate(appointment_time);
</script>
</body>
</html>