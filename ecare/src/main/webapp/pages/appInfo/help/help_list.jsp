    ...
    <div id="main-content" class="clearfix">
        ...
        <div id="page-content" class="clearfix">
            <div class="row-fluid">
                <div class="row-fluid">
                    <!-- 检索  -->
                    <form action="${pageContext.request.contextPath}/appInfo/help/findList.do?am=${mPd.am}&bm=${mPd.bm}&pm=${topMPd}" method="post" name="customerForm" id="customerForm">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select id="type" name="type">
                                            ...
                                            <option value="business" <c:if test="${pd.type=='business'}">selected</c:if>>商户</option>
                                        </select>
                                    </td>
                                    ...
                                    <table id="table_report" class="table table-striped table-bordered  table-hover">
                                        ...
                                        <c:choose>
                                            <c:when test="${not empty helpList}">
                                                <c:forEach items="${helpList}" var="var" varStatus="vs">
                                                    <tr ondblclick="findInfo('${var.id }')">
                                                        ...
                                                        <td>
                                                            <c:if test="${var.type =='family'}">家庭</c:if>
                                                            <c:if test="${var.type =='business'}">商户</c:if>
                                                        </td>
                                                        <td>${var.title }</td>
                                                        <td>
                                                            <c:if test="${var.status =='N'}"><span style="color:grey">禁用</span></c:if>
                                                            <c:if test="${var.status =='Y'}"><span style="color:green">启用</span></c:if>
                                                        </td>
                                                        <td><fmt:formatDate value="${var.create_time}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                                                        <td id="exclude_1_${var.id }" style="">
                                                            <c:if test="${fn:contains(power,'help_edit')&& var.status == 'N'}">
...
<script type="text/javascript">
    ...
    //修改
    function edit(id){
        art.dialog.open('${pageContext.request.contextPath}/appInfo/help/toedit.do?id='+id,{
            title:'编辑',
            width:550,
            height:500,
            lock: true
...
//				var data = $('input:checkbox[id^=check_]:checked');
//				if(data.length==1){
        art.dialog.open('${pageContext.request.contextPath}/appInfo/help/findInfo.do?id='+id,{
            title:'查看详情',
            width:750,
            height:400,
            lock: true,
