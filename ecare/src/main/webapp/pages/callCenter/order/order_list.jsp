    ...
    <div id="main-content" class="clearfix">
        ...
        <div id="page-content" class="clearfix">
            <div class="row-fluid">
                <div class="row-fluid">
                    <!-- 检索  -->
                    <form action="${pageContext.request.contextPath}/customer/customer/findList.do?am=${mPd.am }&bm=${mPd.bm }" method="post" name="userForm" id="userForm">
                                                ...
                                                <c:forEach items="${userList}" var="var" varStatus="vs">
                                                    <tr>
                                                        ...
                                                        <td id="exclude_1_${var.id }">
                                                            <c:if test="${fn:contains(power,'telemarketer_edit')}">
                                                                <a class='btn btn-mini btn-info' title="查看" onclick="findInfoById('${var.id }');"><i class='icon-user'></i></a>
                                                            </c:if>
                                                                <%--<c:if test="${fn:contains(power,'telemarketer_edit')}">--%>
...
<script type="text/javascript">
    ...
    function findInfo(){
        var data = $('input:checkbox[id^=check_]:checked');
        if(data.length==1){
            art.dialog.open('${pageContext.request.contextPath}/customer/customer/findInfo.do?id='+data.val(),{
                title:'个人信息',
                width:650,
                height:500,
                lock: true
    ...
    function setServerTypes(){
        var data = $('input:checkbox[id^=check_]:checked');
        if(data.length==1){
            art.dialog.open('${pageContext.request.contextPath}/customer/customer/toServerCategory.do?user_id='+data.val(),{
                title:'设置用户组织',
                width:350,
                height:500,
                lock: true
