...
<script type="text/javascript">
    ...
    function all_clear(){
        bootbox.confirm("确定要清除全部数据吗?清除之后不可恢复！", function(result) {
            if(result) {
                var url = "${pageContext.request.contextPath}/callCenter/telemarketer/clearUser.do?flag=all_clear";
                $.get(url,function(data){
                    if(data){
                        //document.location.reload();
                        nextPage('${page.currentPage}');
    ...
    function all_revert(){
        bootbox.confirm("确定要还原全部数据吗？", function(result) {
            if(result) {
                var url = "${pageContext.request.contextPath}/callCenter/telemarketer/revertUser.do?flag=all_revert";
                $.get(url,function(data){
                    if(data){
                        //document.location.reload();
                        nextPage('${page.currentPage}');
    ...
    function findInfo(id){
        art.dialog.open('${pageContext.request.contextPath}/callCenter/telemarketer/findInfo.do?id='+id,{
            title:'查看详情',
            width:550,
            height:500,
            lock: true
