			...
			<div id="main-content" class="clearfix">
					...
					<div id="page-content" class="clearfix">
						  ...
						  <div class="row-fluid">
							<div class="row-fluid">
									<!-- 检索  -->
									<form action="${pageContext.request.contextPath}/common/commonRole/findList.do?classify=${pd.classify }&am=${mPd.am }&bm=${mPd.bm }" method="post" name="roleForm" id="roleForm">
										...
										<c:choose>
											<c:when test="${not empty roleList}">
												<c:forEach items="${roleList}" var="role" varStatus="vs">
													<tr>
														<td id="exclude_0_${role.id }"><input type="checkbox" style="opacity: 1;" value="${role.id}" name="ids" id="check_${role.id }" onchange="myCheck('${role.id }');"> </td>
														<td>${role.name }</td>
														...
														<td id="exclude_0_${role.id }" style="">
															<c:if test="${fn:contains(power,'role_edit')}">
																<a class='btn btn-mini btn-info' title="编辑" onclick="update('${role.id }');"><i class='icon-edit'></i></a>
															</c:if>
															<c:if test="${fn:contains(power,'role_del')}">
																<a class='btn btn-mini btn-danger' title="删除" onclick="del('${role.id }');"><i class='icon-trash'></i></a>
															</c:if>
														</td>
													</tr>
												</c:forEach>
...
<script type="text/javascript">
	...
	//新增
	function add(){
		art.dialog.open('${pageContext.request.contextPath}/common/commonRole/toadd.do?classify=${pd.classify }',{
	   	 	title:'新增',
			width:640,
    		height:380,
    		lock: true
	...
	function update(roleid){
		if(roleid==''){
			...
			if(v.length==1){
				var id = v.val();
				art.dialog.open('${pageContext.request.contextPath}/common/commonRole/toedit.do?id='+id,{
		    	 	title:'编辑',
		    	 	width:640,
		    	 	height:380,
		        	lock: true
			...
			art.dialog.open('${pageContext.request.contextPath}/common/commonRole/toedit.do?id='+roleid,{
	    	 	title:'编辑',
	    	 	width:640,
	    	 	height:380,
	        	lock: true
	...
	function findinfo(){
		...
		if(id.length==1){
			art.dialog.open('${pageContext.request.contextPath}/common/commonRole/findInfo.do?id='+id.val(),{
	    	 	title:'查看详情',
	    	 	width:640,
	    	 	height:380,
	        	lock: true
	...
	function del(delid){
		bootbox.confirm("确定要删除选中数据吗？", function(result) {
			if(result){
			   $.ajax({
					url : '${pageContext.request.contextPath}/common/commonRole/delete.do?id='+delid+'&remove_logo=Y',
					dataType : 'json',
					success : function(r) {
						saveSuccess();
					}
	...
	function findDustbin(){
		art.dialog.open('${pageContext.request.contextPath}/common/commonRole/findTrash.do',{
	   	 	title:'回收站',
	   		width:1000,
 			height:540,
 			lock: true,
	...
	function add_menu_role(){
		...
		if(v.length==1){
			var id = v.val();
			art.dialog.open('${pageContext.request.contextPath}/common/commonRole/toaddMenuRole.do?classify=${pd.classify }&role_id='+id,{
				title:'菜单权限',
				width:280,
	    		height:370,
	    		lock: true
