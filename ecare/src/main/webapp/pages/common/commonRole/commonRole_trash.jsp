...
<html lang="en">
  ...
  <body>
    ...
    <div class="row-fluid">
			<!-- 检索  -->
			<form action="${pageContext.request.contextPath}/common/commonRole/findTrash.do?am=${mPd.am }&bm=${mPd.bm }" method="post" name="roleForm" id="roleForm">
			<!-- 检索  -->
...
<%@include file="../../system/admin/bottom.jsp" %>
		...
		if(id.length>0){
			bootbox.confirm("确定要还原选中的数据吗？", function(result) {
				if(result){
					...
					$.ajax({
						url : '${pageContext.request.contextPath}/common/commonRole/delete.do?id='+ids+'&remove_logo=N',
						dataType : 'json',
						success : function(r) {
	...
	function revertRoleAll(){
		bootbox.confirm("确定要一键还原所有数据？", function(result) {
			if(result){
				$.ajax({
					url : '${pageContext.request.contextPath}/common/commonRole/revertRoleAll.do?remove_logo_N=N&remove_logo=Y',
					dataType : 'json',
					success : function(r) {
						nextPage('${page.currentPage}');
					}
		...
		if(id.length>0){
			bootbox.confirm("确定要清除选中的数据吗?清除之后不可恢复！", function(result) {
				if(result){
					...
					$.ajax({
						url : '${pageContext.request.contextPath}/common/commonRole/clearRole.do?id='+ids+'&flag=clear',
						dataType : 'json',
						success : function(r) {
							nextPage('${page.currentPage}');
						}
	...
	function clearAll(){
			bootbox.confirm("确定要清空所有数据？清除之后不可恢复！", function(result) {
				if(result){
					$.ajax({
						url : '${pageContext.request.contextPath}/common/commonRole/clearRole.do?flag=all_clear',
						dataType : 'json',
						success : function(r) {
							nextPage('${page.currentPage}');
						}
	...
	function findInfo(id){
		art.dialog.open('${pageContext.request.contextPath}/common/commonRole/findInfo.do?id='+id,{
    	 	title:'查看详情',
    	 	width:640,
    	 	height:380,
        	lock: true
