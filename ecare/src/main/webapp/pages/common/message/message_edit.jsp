...
//				  abstract:  "<span style='color:red'>请输入消息内容。</span>",
//				  content:  "<span style='color:red'>请输入消息内容。</span>"
		...
		function isCheck(){
			var id = "${p.recipients_id}";
			var ids = id.split(",");
			for(var i=0;i<ids.length;i++){
				var name = ids[i];
				$("#"+name).attr("checked",'true');
			}
////			$("[name='recipients']").attr("checked",'true');//全选
//			$("[name='recipients']").attr("checked",'true');//全选
		}
		/**
		 * 复选框选中事件
  ...
  <body>
    <form action="${pageContext.request.contextPath}/common/message/update.do" name="cdForm" id="cdForm" method="post" enctype="multipart/form-data">
		...
		<table width="90%">
			...
			<tr>
				...
				<td>
					<input type="checkbox" name="recipients" id="telemarketer"  value="telemarketer" style="width: 30px;margin-bottom: 8px" onclick="jqchk()"/>客服
					<input type="checkbox" name="recipients" id="partners"  value="partners" style="width: 30px;margin-bottom: 8px" onclick="jqchk()"/>商家
					<input type="checkbox" name="recipients" id="server" value="server" style="width: 30px;margin-bottom: 8px" onclick="jqchk()"/>商户
					<input type="checkbox" name="recipients" id="family"  value="family" style="width: 30px;margin-bottom: 8px" onclick="jqchk()"/>家人
					<input type="hidden" name = "recipients_id" id="recipients_id" value="${p.recipients_id}"/>
				</td>
			</tr>
			<tr>
				<td width="20%" align="right">简&nbsp;&nbsp;&nbsp;&nbsp;介：<span style="color:#FF0000">*</span></td>
				<td>
					<textarea id="remark" style="width:500px;height:100px;" name="remark">${p.remark}</textarea>
				</td>
			</tr>
			<tr>
				<td width="20%" align="right">内容：</td>
				<td>
					<textarea id="content" style="width:500px;height:300px;" name="content">${p.content}</textarea>
				</td>
			</tr>
