	...
	<div id="page-content" class="clearfix">
		<div class="row-fluid">
			<div class="row-fluid">
				<!-- 检索  -->
				<form
					action="${pageContext.request.contextPath}/common/message/findTrash.do?am=${mPd.am }&bm=${mPd.bm }"
									...
									<c:forEach items="${list}" var="var" varStatus="vs">
										<tr>
											<td style="width: 20px;"><input type="checkbox"
												...
												onchange="myCheck('${var.id }');">
											</td>
											<td>${var.title }</td>
<!-- 														<td>${var.send_user }</td> -->
<!-- 														<td>${var.accept_user }</td> -->
														<td>
	...
	<script type="text/javascript">
			...
			if (data.length > 0) {
				...
				bootbox
						.confirm(
								...
								function(result) {
									if (result) {
										var url = "${pageContext.request.contextPath}/common/message/clearMessage.do?ids="
												+ ids + "&flag=clear";
										$
												.get(
														url,
			...
			if (data.length > 0) {
				...
				bootbox
						.confirm(
								...
								function(result) {
									if (result) {
										var url = "${pageContext.request.contextPath}/common/message/revertMessage.do?ids="
												+ ids + "&flag=revert";
										$
												.get(
														url,
		...
		function all_clear() {
			bootbox
					.confirm(
							...
							function(result) {
								if (result) {
									var url = "${pageContext.request.contextPath}/common/message/clearMessage.do?flag=all_clear";
		...
		function all_revert() {
			bootbox
					.confirm(
							...
							function(result) {
								if (result) {
									var url = "${pageContext.request.contextPath}/common/message/revertMessage.do?flag=all_revert";
									$.get(url, function(data) {
										if (data) {
											//document.location.reload();
											nextPage('${page.currentPage}');
		...
		function findInfo(id) {
			art.dialog.open(
					'${pageContext.request.contextPath}/common/message/findInfo.do?id='
							+ id, {
						title : '查看详情',
						width : 550,
						height : 500,
