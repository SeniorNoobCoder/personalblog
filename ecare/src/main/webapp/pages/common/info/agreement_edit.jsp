...
//					  },
                    type: "<span style='color:red'>请选择接收人。</span>",
...
<body>
<form action="${pageContext.request.contextPath}/common/info/updateAgreement.do" name="cdForm" id="cdForm" method="post" enctype="multipart/form-data">
    <c:if test="${pd.label!='b'}">
        <div id="zhongxin">
            <table width="90%">
                <tr>
                    <td width="20%" align="right">标&nbsp;&nbsp;&nbsp;&nbsp;题：<span style="color:#FF0000">*</span></td>
                    <td><input type="text" name="title" id="title" placeholder="这里输入标题" title="标题" value="${p.title}"/></td>
                </tr>
                    <%--<tr>--%>
                    ...
                    <%--</tr>--%>
                <tr>
                    <td width="20%" align="right">类型：<span style="color:#FF0000">*</span></td>
                    <td>
                        <input type="radio" name="typeName"  id="typeName" value="server" style="width: 30px;margin-bottom: 8px" onclick="jqchk()" <c:if test="${p.user_type=='server'}">checked</c:if>/>商户
                        <input type="radio" name="typeName"  id="typeName" value="family" style="width: 30px;margin-bottom: 8px" onclick="jqchk()" <c:if test="${p.user_type=='family'}">checked</c:if>/>家人
                        <input type="hidden" id="user_type" name="user_type" value="">
                    </td>
                </tr>
                <tr>
                    <td width="20%" align="right">修改内容：<span style="color:#FF0000">*</span></td>
                    <td>
                        <textarea id="editor" type="text/plain" style="width:700px;height:300px;" name="content">${p.content}</textarea>
                    </td>
                </tr>
                <tr>
                    <td style="text-align: center;" colspan="2">
                        ...
                        <a class="btn btn-mini btn-primary" onclick="savePublish();">保存并发布</a>
                        <input type="hidden" id="isPublish" name="isPublish" value="">
