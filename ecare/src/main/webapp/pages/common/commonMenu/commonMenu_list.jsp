...
<%@include file="commonMenu_ztree.jsp" %>
		<script type="text/javascript">

		var power = '${power}';
			//设置图标
			function getIcon(id){
					art.dialog.open('${pageContext.request.contextPath}/common/commonMenu/toMenuList.do?id='+id,{
				   	 	title:'选择图标',
						width:650,
			    		height:120,
			    		lock: true
			...
			function showButton(){
				...
				var id =  $("#tree_id").val();
					$.ajax({
			            type: "POST",
			            url: "${pageContext.request.contextPath}/common/commonMenu/findList.do?parent_id="+id+'&type='+type,
			            dataType: "json",
			...
			function addMenu(){
				...
				if(id==''||id==null){
					...
					if(type=='button'){
					    ...
					    var url = "${pageContext.request.contextPath}/common/commonMenu/getListNum.do?type=button&parent_id="+id;
					    $.ajax({
				            type: "POST",
				            url: url,
				            data: '',
					    ...
					    if(buttonNum==0){
							art.dialog.open('${pageContext.request.contextPath}/common/commonMenu/toadd.do?classify=${pd.classify }&type=menu&id='+id+'&level='+level+'&name='+name,{
						   	 	title:'新增',
								width:550,
					    		height:400,
					    		lock: true
			...
			function addButton(){
				...
				if(id==''||id==null){
					...
					if(type=='button'){
						...
						var buttonNum=0;
						//查询当前菜单下 是否有按钮,有按钮则不允许添加下级菜单
					    var url = "${pageContext.request.contextPath}/common/commonMenu/getListNum.do?classify=${pd.classify }&type=menu&parent_id="+id;
					    $.ajax({
				            type: "POST",
				            url: url,
				            data: '',
					    ...
					    if(buttonNum==0){
							art.dialog.open('${pageContext.request.contextPath}/common/commonMenu/toadd.do?type=button&id='+id+'&level='+level+'&name='+name,{
						   	 	title:'新增',
								width:550,
					    		height:400,
					    		lock: true
			...
			//修改
			function edit(id){
				art.dialog.open('${pageContext.request.contextPath}/common/commonMenu/toedit.do?id='+id,{
			   	 	title:'编辑',
					width:550,
		    		height:350,
		    		lock: true
			...
			function del(id){
					bootbox.confirm("确定要删除该记录?", function(result) {
						if(result) {
							var url = "${pageContext.request.contextPath}/common/commonMenu/del.do?id="+id;
