  ...
  <body>
    <div id="page-content" class="clearfix">
	  <div class="row-fluid">
		<div class="row-fluid">
				<!-- 检索  -->
				<form action="${pageContext.request.contextPath}/customer/device/findTrash.do" method="post" name="deviceForm" id="deviceForm">
	...
	<script type="text/javascript">
		...
		function myClear(){
			...
			if(data.length>0){
				...
				if(result) {
					var url = "${pageContext.request.contextPath}/customer/device/clearTrash.do?ids="+ids+"&flag=clear";
		...
		function revert(){
			...
			if(data.length>0){
				...
				if(result) {
					var url = "${pageContext.request.contextPath}/customer/device/revertTrash.do?ids="+ids+"&flag=revert";
					$.get(url,function(data){
						if(data){
							//document.location.reload();
							nextPage(${page.currentPage});
		...
		function all_clear(){
			bootbox.confirm("确定要清除全部数据吗?清除之后不可恢复！", function(result) {
				if(result) {
					var url = "${pageContext.request.contextPath}/customer/device/clearTrash.do?flag=all_clear";
					$.get(url,function(data){
						if(data){
							//document.location.reload();
							nextPage(${page.currentPage});
		...
		function all_revert(){
			bootbox.confirm("确定要还原全部数据吗？", function(result) {
				if(result) {
					var url = "${pageContext.request.contextPath}/customer/device/revertTrash.do?flag=all_revert";
					$.get(url,function(data){
						if(data){
							//document.location.reload();
							nextPage(${page.currentPage});
		...
		function findInfo(id){
			art.dialog.open('${pageContext.request.contextPath}/customer/device/findInfo.do?id='+id,{
		   	 	title:'查看详情',
				width:550,
	    		height:500,
	    		lock: true
