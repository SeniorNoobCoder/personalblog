...
public class HDUtil {
	...
	private static HDUtil hdUtil;
	
	public static HDUtil getInstance() {
		if(hdUtil==null) hdUtil = new HDUtil();
		return hdUtil;
	}
	...
	private static void loadHanData() throws IOException {
		InputStream in = HDUtil.class.getResourceAsStream(HAN_DATA_FILE);
	...
	public static String getPyjc(String str, boolean useHanFormat,boolean isDaXie){
		if (str == null) {
			return "";
		}
		boolean lastBlank = true;
				...
				if (!pyList.isEmpty()) {
					if (!lastBlank) {
						sb.append(" ");
						 ...
						 if(isDaXie){
							 sb.append(pyList.get(0).substring(0, 1).toUpperCase());
						 }else{
							 sb.append(pyList.get(0).substring(0, 1).toLowerCase());
						 }
					}
					lastBlank = true;
				}
	...
	public static void main(String[] args) {
		char ch = '郑';
		System.out.println(ch + "的拼音（中式注音）为：" + HDUtil.getPY(ch, true));
		System.out.println(ch + "的拼音（英式注音）为：" + HDUtil.getPY(ch, false));
		System.out.println(ch + "的部首为　　　　　　：" + HDUtil.getBS(ch));
		System.out.println(ch + "的部首笔画为　　　　：" + HDUtil.getBH(HDUtil.getBS(ch)));
		System.out.println(ch + "的笔画顺序为　　　　：" + HDUtil.getBH(ch));
		System.out.println(ch + "的笔画数为　　　　　：" + HDUtil.getBH(ch).length());
		System.out.println();
		ch = '大';
		System.out.println(ch + "的拼音（中式注音）为：" + HDUtil.getPY(ch, true));
		System.out.println(ch + "的拼音（英式注音）为：" + HDUtil.getPY(ch, false));
...
//	public void Test(){
//		String ch = "计划经营部";
//		System.out.println(ch + "的拼音（中式注音）为：" + HDUtil.getPyjc(ch, true,false));
//	}
