...
@Service
@Transactional
public class ServerCategoryDetailServiceImpl implements ServerCategoryDetailService {
    Logger logger = Logger.getLogger(ServerCategoryServiceImpl.class);
    String mapperUrl = "com.ecare.web.server.categoryDetail.";
    @Autowired
     ...
     * @param pd
     */
    public void saveCategoryDetail(PageData pd) {
        try {
            dao.save(mapperUrl + "saveCategoryDetail", pd);
        } catch (Exception e) {
            logger.error(e);
            e.printStackTrace();
        }
