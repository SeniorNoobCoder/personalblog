...
@Service
@Transactional
public class MallPhotoServiceImpl implements MallPhotoService {
	Logger logger = Logger.getLogger(MallPhotoServiceImpl.class);
	String mapperUrl = "com.ecare.web.appInfo.mallPhoto.";
	@Autowired
	 ...
	 */
	public Integer findNum(PageData pd) {
		Integer num = 0;
		try {
	...
	@SuppressWarnings("unchecked")
	@Transactional
	public void clearMallPhoto(PageData pd) {
		try {
	...
	@SuppressWarnings("unchecked")
	@Transactional
	public void revertMallPhoto(PageData pd) {
		try {
			String flag = pd.getString("flag");
